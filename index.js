...
function getMarkdownFilePaths(directory) {
  const ignoreList = ['.git', '_layouts', 'assets', 'node_modules'];
...
function render(markdownFilePaths, problems) {
  ...
  const isProblemFound = markdownFilePaths.some(
    (file) => problems[file].length
  );

  if (isProblemFound) {
    console.log(
      colors.red(
        `🛑 Some Markdown formatting problems were found and should be resolved. See the notes above for details.`
      )
    );
  } else {
